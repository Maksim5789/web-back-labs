...
def congratulations():
    ...
    if age >= 18:  # Проверка на возраст
        if gender == 'male':
            greeting = f"Поздравляю вас, {name}! Желаю, чтобы воплощались все мечты и достигались цели, а стремление к ним было вдохновляющим и интересным!"
        else:
            greeting = f"Поздравляю вас, {name}! Желаю оставаться такой же красивой, милой и прекрасной!"
    else:
        if gender == 'male':
